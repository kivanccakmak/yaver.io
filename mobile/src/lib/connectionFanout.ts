 ...
 *
 * ── This file is the TWIN of web/lib/connectionFanout.ts ─────────────────────
 *
 * Byte-for-byte the same logic, kept in sync by connectionFanout.test.ts, which
 * compares the two sources. Native surfaces do not inherit an RN fix and RN
 * surfaces do not inherit a web one, so the only defence against drift is a
 * test that reads both files. This module is imported by mobile, tablet, car
 * and glass — they all consume the same DeviceContext — so one edit here covers
 * every RN surface.
 *
