...
// there is one.
//
// Mirrored, deliberately, from backend/convex/deviceIdentityMerge.ts (also in
// web/lib/). EVERY surface collapses these rows again over its own snapshot —
// the phone in DeviceContext.tsx::collapseAliasDevices, the browser in
// use-devices.ts — and each carried its own `||` chain with a DIFFERENT set of
// clauses. A fix in one is not a fix.
...
// module resolution, and here it keeps the copies byte-comparable.
