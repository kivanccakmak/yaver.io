  ...
  return (
    <View style={[styles.safeArea, { backgroundColor: c.bg }]}>
      ...
      >
        ...
        {/* The "Dogfood Yaver" panel used to live here. Removed 2026-08-02,
            replaced by Attach Mode above. It was dead in three ways at once:
              - wrapped in {!LEAN_SETTINGS_SURFACE && …} where the flag is a
                hardcoded `true`, so it never rendered;
              - its local saveDogfoodConfig() wrote only {repoDir, prompt} to
                the same key dogfoodConfig.ts stores {repoDir, prompt, runner,
                mode} in, so editing the path here silently reset the Dogfood
                tab's runner and mode;
              - "Kick Hermes refresh" dispatched a CODING TASK asking the agent
                to prepare a Hermes flow for Yaver — rendering routed through
                the runner system (which devLane.ts forbids), aimed at the one
                target the recursion guard 409s. A button that could not
                succeed. */}
