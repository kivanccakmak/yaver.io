...
/// FailureSignals — runner-failure classification for visionOS.
///
...
///
/// WHY VISIONOS NEEDS THIS. A spatial surface has no terminal to fall back to
/// and no easy way to paste a raw provider dump somewhere useful, so an
/// unclassified failure is a dead end in a headset. Naming the cause is the
/// difference between a fixable state and taking the device off.
///
...
enum FailureSignals {
    ...
    /// One short line per cause. A spinner is the only unacceptable answer.
