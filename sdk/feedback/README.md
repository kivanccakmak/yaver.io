...
The Feedback SDK ships across **React Native (mobile)**, **Flutter (mobile)**, **Web (browser)**, **Kotlin (native Android)**, **Swift (native iOS)**, and **Python (server-side / scripts)**. They share one auth model, one Yaver backend, and one local-first philosophy.
