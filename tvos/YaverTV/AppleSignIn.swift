...
// Guarded for the same reason OAuthSignIn.swift is: the tvOS app consolidated
// sign-in into device-code + email and no longer compiles these files, but the
// visionOS target shares the tvos/YaverTV/ directory and still needs them. The
// guard keeps the tvOS build clean while the headset keeps its fast path.
#if !os(tvOS)
