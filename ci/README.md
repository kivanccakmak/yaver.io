...
This directory holds everything needed to run Yaver integration tests against a real remote Linux box — hybrid mode with Ollama, remote-worker, `ops` verb routing, OAuth — on dedicated Hetzner infrastructure rather than GitHub-hosted runners.
...
- An always-on CI runner. GitHub-hosted runners are cheaper and simpler for unit/lint/typecheck work — this box is strictly for tests that need a real remote Linux host (integration, Cloudflare/Tailscale/relay connectivity, hybrid-mode with Ollama, OAuth callbacks against mocks).
...
- Keep host base URLs, device ids, and account passwords in secrets only. Do not print session ids or host base URLs into logs unless they are explicitly masked.
