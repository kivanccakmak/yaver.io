...
export const PREDEFINED_RUNNERS = [
  {
    ...
    name: "Claude Code",
    command: "claude",
    args: JSON.stringify(["-p", "{prompt}", "--output-format", "stream-json", "--verbose", "--include-partial-messages", "--tools", "Bash", "--dangerously-skip-permissions"]),
  ...
  },
  {
    ...
    name: "OpenAI Codex",
    command: "codex",
    args: JSON.stringify(["--full-auto", "{prompt}"]),
