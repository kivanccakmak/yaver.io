...
const defaultProjectPendingFeedbackLimit = 80;
...
async function countPendingFeedbackForIndex(
  ctx: any,
  tableIndex: "by_owner_status" | "by_owner_project_status",
  ...
  for (const status of pendingFeedbackStatuses) {
    const rows = await ctx.db
      ...
      .withIndex(tableIndex, (q: any) => {
        const owner = q.eq("ownerUserId", ownerUserId);
        return projectSlug ? owner.eq("projectSlug", projectSlug).eq("status", status) : owner.eq("status", status);
      })
      .collect();
...
async function enforceFeedbackQueueLimits(
  ctx: any,
  args: { ownerUserId: Id<"users">; projectSlug: string; now: number },
) {
  const ownerLimit = feedbackPendingLimit(process.env.YAVER_FEEDBACK_OWNER_PENDING_LIMIT, defaultOwnerPendingFeedbackLimit);
  const projectLimit = feedbackPendingLimit(process.env.YAVER_FEEDBACK_PROJECT_PENDING_LIMIT, defaultProjectPendingFeedbackLimit);
  const [ownerPending, projectPending] = await Promise.all([
    countPendingFeedbackForIndex(ctx, "by_owner_status", args.ownerUserId, undefined, args.now),
    countPendingFeedbackForIndex(ctx, "by_owner_project_status", args.ownerUserId, args.projectSlug, args.now),
  ]);
  ...
  if (feedbackPendingLimitExceeded({ pendingCount: projectPending, limit: projectLimit })) {
    throw new Error(`feedback queue limit reached for project: ${projectPending}/${projectLimit}`);
  }
}
...
async function sdkContext(ctx: any, sdkTokenHash: string): Promise<{
  ...
  if (row.delegatedGuestUserId) {
    throw new Error("Legacy delegated SDK tokens are invalid; create an owner SDK token");
  }
  return {
...
export const createFromSdk = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    assertProjectAllowed(projectSlug, sdk.allowedProjects);
...
export const list = query({
  ...
  handler: async (ctx, args) => {
    ...
    const rows: any[] = projectSlug
      ? await ctx.db.query("feedbackWorkItems")
...
export function feedbackRelaySourceLocalTaskCollision(
  ...
  return String(row.ownerUserId) !== String(args.ownerUserId) || String(row.projectSlug) !== String(args.projectSlug);
}
...
export const queueRelaySource = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const now = Date.now();
    const repoUrl = normalizeRelayRepoUrl(args.repoUrl, args.branch);
