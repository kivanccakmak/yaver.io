...
// there is one.
//
// Mirrored, deliberately, in web/lib/deviceIdentityMerge.ts — web collapses the
// same rows a second time client-side (web/lib/use-devices.ts), so a fix in one
// is not a fix. Both are covered by tests that share these fixtures.

// TYPE-ONLY import on purpose. Convex modules resolve siblings extensionless
// ("./aliasShadowing"), which Node's ESM loader cannot resolve when the unit
// test runs this file directly — and switching the runtime import to
// "./aliasShadowing.ts" to please the test would change what the Convex bundler
// sees at deploy time. A type import is erased by the type-stripper, so the
// PREDICATE arrives as a parameter (`relate`) instead: one implementation, one
// home (aliasShadowing.ts), and no runtime edge between the two modules.
