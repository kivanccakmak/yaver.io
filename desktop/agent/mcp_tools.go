...
func (s *HTTPServer) getMCPToolsList() interface{} {
			...
			"inputSchema": map[string]interface{}{
				...
				"properties": map[string]interface{}{
					...
					"model": map[string]interface{}{
						"type":        "string",
						"description": "Model id forwarded to the runner (e.g. claude-opus-4-8, gpt-5.6-sol, or any opencode-configured provider/model). Empty = Convex-backed Yaver default.",
					},
	...
	appDevTools := []map[string]interface{}{
		...
		{"name": "testflight_promote", "description": "Make an uploaded build available to a TestFlight beta group — the App Store Connect analog of the Play 'promote release' action. POLICY-AWARE: an EXTERNAL group's first build goes through Beta App Review, so promoting to an external group is refused with a POLICY_REFUSED error unless `confirm='external'` acknowledges it; internal groups need no confirmation. Does not upload — promotes an existing build. bundle_id required; buildNumber optional (defaults to the newest build); groupName optional (default 'External', created if missing). OWNER-ONLY (mutates TestFlight availability).", "inputSchema": map[string]interface{}{"type": "object", "required": []string{"bundle_id"}, "properties": map[string]interface{}{"bundle_id": map[string]interface{}{"type": "string", "description": "Bundle ID (e.g. io.yaver.mobile)"}, "buildNumber": map[string]interface{}{"type": "string", "description": "Build number/version to promote (default: newest build)"}, "groupName": map[string]interface{}{"type": "string", "description": "Target TestFlight beta group (default 'External', created if missing)"}, "confirm": map[string]interface{}{"type": "string", "description": "Set 'external' to acknowledge Beta App Review when promoting to an EXTERNAL group."}}}},
		...
		{"name": "playstore_promote", "description": "Promote an already-uploaded Play versionCode to a target track (internal → alpha → beta → production). Does NOT re-upload the AAB — assigns the existing build to the track and commits, like the Play Console 'Promote release' button. POLICY-AWARE: the staged-release ladder is enforced — promoting to a track the build has not passed through (e.g. internal → production) is refused with a POLICY_REFUSED error unless `confirm` matches the target (explicit human sign-off for skips / production). Play's declaration gates (Health-apps / Data-safety) surface as DECLARATION_BLOCKED with the exact console step; production reminds to check device support. Owner-only.", "inputSchema": map[string]interface{}{"type": "object", "required": []string{"versionCode", "track"}, "properties": map[string]interface{}{"package": map[string]interface{}{"type": "string", "description": "Package name (default io.yaver.mobile)"}, "versionCode": map[string]interface{}{"type": "string", "description": "The versionCode to promote (the release is already uploaded on a lower track)."}, "track": map[string]interface{}{"type": "string", "enum": []string{"internal", "alpha", "beta", "production"}, "description": "Target track to promote into."}, "releaseNotes": map[string]interface{}{"type": "string", "description": "Optional 'lang: text' pairs joined by |, e.g. 'en-US: Tablet studio + runtime UX'."}, "confirm": map[string]interface{}{"type": "string", "description": "Explicit sign-off required when the ladder is skipped (e.g. internal → production). Set to the target track name to acknowledge the staged-release policy."}}}},
	...
	platformTools := []map[string]interface{}{
		...
		{"name": "runner_auth_browser_start", "description": "Start the interactive plan-OAuth browser/device-auth login flow for Claude Code or Codex on the local or a remote Yaver machine. Uses the user's Claude Pro/Max or ChatGPT Plus/Pro account on their own device; no API key is requested. Set confirm=true only for an explicit user-requested reauth/account switch when credentials already exist.", "inputSchema": map[string]interface{}{"type": "object", "required": []string{"runner"}, "properties": map[string]interface{}{"device_id": map[string]interface{}{"type": "string", "description": "Optional remote device ID"}, "runner": map[string]interface{}{"type": "string", "enum": []string{"claude", "claude-code", "codex"}}, "confirm": map[string]interface{}{"type": "boolean", "description": "Explicitly replace/re-auth an existing credential. Default false; use only after the user requested it."}}}},
		...
		},
		{
			...
			"inputSchema": map[string]interface{}{
				...
				"properties": map[string]interface{}{
					...
					"confirm": map[string]interface{}{
						"type":        "boolean",
						"description": "Explicitly replace/re-auth an existing runner credential. Default false; use only after the user requested it.",
					},
				},
			},
		},
	...
	runtimeRuntimeTools := []map[string]interface{}{
		...
		},
		{
			...
			"description": "Run a session-level command (boot re-attach, project rerender/relaunch, launch-app, launch-feedback). The compatibility command run-guest rebuilds and relaunches the selected RN/Expo project app into the active simulator/emulator stream, including tvOS when available; launch-app requires bundleId; boot is idempotent.",
			"inputSchema": map[string]interface{}{
				"type":     "object",
	...
	browserTools := []map[string]interface{}{
		...
		},
		{
			...
			"description": "Start a GENERIC human-in-the-loop co-browse with a persistent profile, navigate to a URL, and return frame/input HTTP paths so a human can solve a captcha or log in remotely. Set headful=true to also show the browser on the host. Automation resumes on the SAME session (cookies/auth persist on disk) afterward.",
			"inputSchema": map[string]interface{}{
				"type":     "object",
				"required": []string{"url"},
				"properties": map[string]interface{}{
					...
					"headful":    map[string]interface{}{"type": "boolean", "description": "Show the browser window visibly on the host (default: false; remote frame/input still works headlessly)"},
