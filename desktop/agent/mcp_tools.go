...
func (s *HTTPServer) getMCPToolsList() interface{} {
	...
	appDevTools := []map[string]interface{}{
		...
		{"name": "testflight_promote", "description": "Make an uploaded build available to a TestFlight beta group — the App Store Connect analog of the Play 'promote release' action. OWNER-ONLY (mutates TestFlight availability; external groups see the build after Beta App Review for the first build). Does not upload — promotes an existing build. bundle_id required; buildNumber optional (defaults to the newest build); groupName optional (default 'External', created if missing).", "inputSchema": map[string]interface{}{"type": "object", "required": []string{"bundle_id"}, "properties": map[string]interface{}{"bundle_id": map[string]interface{}{"type": "string", "description": "Bundle ID (e.g. io.yaver.mobile)"}, "buildNumber": map[string]interface{}{"type": "string", "description": "Build number/version to promote (default: newest build)"}, "groupName": map[string]interface{}{"type": "string", "description": "Target TestFlight beta group (default 'External', created if missing)"}}}},
		...
		{"name": "playstore_promote", "description": "Promote an already-uploaded Play versionCode to a target track (internal → alpha → beta → production). Does NOT re-upload the AAB — assigns the existing build to the track and commits, like the Play Console 'Promote release' button. Use for the staged release ladder: internal → beta → production. A Play declaration gate (e.g. the Health-apps form) returns a named error with the exact console step to unblock it.", "inputSchema": map[string]interface{}{"type": "object", "required": []string{"versionCode", "track"}, "properties": map[string]interface{}{"package": map[string]interface{}{"type": "string", "description": "Package name (default io.yaver.mobile)"}, "versionCode": map[string]interface{}{"type": "string", "description": "The versionCode to promote (the release is already uploaded on a lower track)."}, "track": map[string]interface{}{"type": "string", "enum": []string{"internal", "alpha", "beta", "production"}, "description": "Target track to promote into."}, "releaseNotes": map[string]interface{}{"type": "string", "description": "Optional 'lang: text' pairs joined by |, e.g. 'en-US: Tablet studio + runtime UX'."}}}},
