...
func TestPathAllowedByScopes(t *testing.T) {
	tests := []struct {
		...
		{"/browser-shortcuts/preflight", []string{"browser-shortcut"}, true},
		{"/dev/build-native", []string{"browser-shortcut"}, true},
		{"/exec", []string{"browser-shortcut"}, false},
...
func TestCompanionSessionAllowed(t *testing.T) {
	tests := []struct {
		...
		{http.MethodPost, "/project/start", "tv", true},
