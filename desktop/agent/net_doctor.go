...
func RunNetDoctor(ctx context.Context, opts NetDoctorOptions) NetDoctorReport {
	...
	switch {
	case cf || goog || cf6 || goog6:
		inet.Status = NetOK
		best := int64(-1)
		for _, ms := range []int64{cfMs, googMs, cf6Ms, goog6Ms} {
			if ms >= 0 && (best < 0 || ms < best) {
				best = ms
			}
		}
		family := "dual-stack"
		if (cf6 || goog6) && !(cf || goog) {
			family = "IPv6-only"
		} else if (cf || goog) && !(cf6 || goog6) {
			family = "IPv4-only"
		}
		inet.Metrics["family"] = family
		inet.Detail = fmt.Sprintf("Reached public internet by IP (%d ms, %s) — raw connectivity is up.", best, family)
		if gwLayer.Status == NetFail {
			// A successful public TCP operation disproves a broken gateway.
			// Some routers simply ignore ICMP echo requests.
