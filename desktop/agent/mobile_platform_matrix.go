	...
	return mobilePlatformMatrixReport{
		...
		Surfaces: []mobilePlatformSurface{
			...
			surface(mobilePlatformSurface{
				...
				StoreTarget: "playstore", DeployTarget: "android-auto", Script: "scripts/deploy-android-auto.sh", QueueTargets: []string{"android-auto", "playstore"},
				Validation:    []string{"deep-build-doctor", "autotest-cdp", "autotest-selenium", "android-device", "android-auto-dhu"},
				Notes:         []string{"Ships through the Android mobile Play artifact after validating the Automotive descriptor, MessagingStyle native bridge, RemoteInput reply receiver, and manifest metadata."},
				Limitations:   []string{"Play review still requires Android Auto/car quality declarations and Desktop Head Unit verification for the messaging notification surface."},
				...
				NextSteps:     []string{"Run scripts/deploy-android-auto.sh for preflight, then verify the MessagingStyle path in Desktop Head Unit before marking the car surface generally available."},
			}),
			surface(mobilePlatformSurface{
			...
			surface(mobilePlatformSurface{
				...
				Notes:         []string{"Runs headset-specific static checks, verifies the visionOS SDK is installed, and checks the iOS artifact remains Vision Pro-compatible when no native visionOS project exists."},
				Limitations:   []string{"Native immersive visionOS upload still needs a configured visionOS project plus App Store Connect platform record; compatible iPad-on-Vision mode ships through the iOS/TestFlight lane."},
			...
			surface(mobilePlatformSurface{
				...
				Notes:         []string{"Runs Android XR/VR manifest and hardware-feature checks, builds the release AAB, and can upload compatible headset-visible artifacts to the Play internal track."},
				Limitations:   []string{"Use scripts/deploy-android-xr.sh --immersive for a strict native OpenXR/Quest gate; Meta Quest Store and Horizon OS submission are not a Play upload."},
