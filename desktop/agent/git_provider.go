...
func seedSandboxWorkspaceYaml(provider, host, username, token, fullName, projectName string) error {
	...
	yamlBody := buildMobileWorkspaceYaml(projectName)
...
// buildMobileWorkspaceYaml declares the predetermined Mobile Workspace stack.
// Git provider selection is intentionally outside the manifest: Yaver Git is
// always available and GitHub/GitLab are account-scoped mirrors.
func buildMobileWorkspaceYaml(projectName string) string {
	return fmt.Sprintf(`# yaver.workspace.yaml — generated by Yaver Mobile Workspace.
version: 1
name: %s
mobile_workspace:
  enabled: true
  origin: yaver-mobile-workspace
workspace:
  root: .
  primary_device: auto
  vault: local
  placement:
    ...
    managed_cloud_fallback: false
apps:
  - name: %s
    path: .
    stack: react-native-expo
    stacks: [react-native-expo, typescript, yaver-serverless, yaver-xml]
    surfaces: [mobile, web]
    test_surfaces: [browser, rn-hermes]
    runtime:
      machine_role: remote-development
