...
func runAuthPair(args []string) {
	if len(args) > 0 && (args[0] == "list" || args[0] == "revoke") {
		fmt.Fprintln(os.Stderr, "additional-account pairing has been removed; pairing installs the primary owner token")
		return
	}
	ttl := 10 * time.Minute
	for i := 0; i < len(args); i++ {
		...
		case "--replace", "--label":
			fmt.Fprintln(os.Stderr, "additional-account pairing options have been removed; pairing installs the primary owner token")
			return
		}
	}
	...
	// Pairing installs the owner's token as the machine's sole account token.
	// Additional-account ledgers were removed with shared-machine access.
	cfg, err := LoadConfig()
	if err != nil || cfg == nil {
		cfg = &Config{}
	}
	...
	fmt.Println("✓ paired — primary owner token installed")
	fmt.Println()
	startServeIfStopped()
}
