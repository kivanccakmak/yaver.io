...
// registerRoutes). Signed companion scopes use the explicit per-verb
// AllowCompanion policy.
...
// boundary upstream (auth() on /mcp). That is why Caller is "owner" here — the
// same construction the `ops` grand-tool uses for its own dispatch.
...
func (s *HTTPServer) handleOps(w http.ResponseWriter, r *http.Request) {
	...
	// Derive caller role from server-stamped companion/capability headers.
...
// opsCallerFromRequest derives the caller role and scope from SERVER-VALIDATED
// middleware headers. Auth is already enforced upstream.
//
...
// rules. Companion and SDK capability callers remain distinct from the owner
// and from each other.
func opsCallerFromRequest(r *http.Request) (caller string, scope string) {
	if capability := strings.TrimSpace(r.Header.Get("X-Yaver-Capability")); capability != "" {
