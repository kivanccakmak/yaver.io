...
| **5.** Mac builder pairing + signaling proxy | `remote_builder.go` + `remote_builder_cmd.go`: local registry under `~/.yaver/builders.json` (mode 0600), `yaver builder {add,list,use,forget,ping}`, optional paired `--local-root` / `--remote-root` checkout mapping, and `yaver serve --builder-platforms=ios,…` opts an agent in as a builder + surfaces `isBuilder` on `/info`. `RemoteRuntimeCapabilities.RemoteBuilders` exposes paired builders to the dashboard. `remote_runtime_dispatch.go`: when Swift / Flutter+iOS hits a non-darwin host, the agent maps the checkout and forwards Create / Get / Offer / Control / Delete to the paired Mac. Browser-shortcut preflight calls the Mac's real capability route for the exact mapped checkout; a registry entry or green `/info` is not treated as simulator readiness. Browser ICE-negotiates direct with the Mac; the Linux box only carries SDP signaling, never RTP. | ✅ shipped (full closer) |
