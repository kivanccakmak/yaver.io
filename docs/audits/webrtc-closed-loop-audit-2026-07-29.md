...
| W8 | `SILENT` | Xcode lists Apple TV device types, but this RN mobile capability probe did not expose a `tvos-simulator` target. A lane absent from inventory is not a named refusal on the surface. | Product gap: surface must expose tvOS as unavailable/unsupported with reason, or provide the lane. |
...
| W10 | `SILENT` | Xcode has modern simulator inventory, but this project capability probe did not expose a visionOS/xrOS target. | Product gap: AR/VR must be represented as supported or named unsupported on every surface. |
