...
| W2 | `PIXELS`/event | Ubuntu Chromium against Mac browser-window: harness waited for `events:open`, navigated the remote tab to a Mac-local logging page through `/remote-runtime/sessions/<id>/control`, and received `browser-log` events for `yaver-log-ok`, `yaver-error-ok`, and uncaught `yaver-throw-ok` over the WebRTC event DataChannel. Same run also decoded pixels: `VERDICT=PIXELS · browser-window:jpeg-dc:webrtc-datachannel-jpeg-v1`. Recording: `/tmp/yaver-webrtc-artifacts/remote-runtime/browser-log-macremote/browser-window.mp4` (`h264`, 1280x800, 42 frames, 10.5s). | Add a UI-level Runtime Console row assertion later; the transport/event contract is now closed-loop proven. |
