...
function shortcutAppUrl(search: string): string {
  const input = new URLSearchParams(search);
  const id = (input.get("id") || input.get("name") || "").trim().slice(0, 100);
  const output = new URLSearchParams();
  if (id) output.set("id", id);
  const qs = output.toString();
  return `yaver://shortcut${qs ? `?${qs}` : ""}`;
}

export default function ShortcutPage() {
  const appUrl = useMemo(() => {
    if (typeof window === "undefined") return "yaver://shortcut";
    return shortcutAppUrl(window.location.search);
  }, []);
