...
export default function VibingPage() {
  ...
  const connect = useCallback(async () => {
    ...
    try {
      ...
      const cfg = (await cfgRes.json()) as { relayServers?: Array<{ httpUrl: string }> };
      const set = (await setRes.json()) as { settings?: { relayPassword?: string; relayUrl?: string; vibingTransport?: string } };
      ...
      <div className="mx-auto max-w-5xl">
        ...
        <div className="mb-6 grid gap-4 sm:grid-cols-2">
          ...
          <div className="rounded-xl border border-surface-800 bg-surface-900 p-4">
            ...
            <div className="rounded-lg border border-surface-700 bg-surface-950 px-3 py-2 text-sm capitalize">{transport === "webrtc" ? "WebRTC (Relay Pro)" : transport === "sse" ? "SSE" : "Auto (SSE → WebRTC)"}</div>
          </div>
        </div>
