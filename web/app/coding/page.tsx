...
import { agentClient, type ConnectionState, type RemoteProject, type Runner, type Task } from "@/lib/agent-client";
import { useAuth } from "@/lib/use-auth";
import { useDevices, type Device } from "@/lib/use-devices";
import { CONVEX_URL } from "@/lib/constants";
import { friendlyTaskPresentation } from "@/lib/_core/taskPresentation";
  ...
  async function run() {
    ...
    try {
      ...
      for (;;) {
        ...
        setTask(current);
        const friendly = friendlyTaskPresentation(current.presentation);
        const assistant = [...friendly].reverse().find((item) => item.kind === "message" && item.role === "assistant")?.text;
        const status = [...friendly].reverse().find((item) => item.kind !== "message")?.text;
        setOutput(assistant || status || current.resultText || current.output.join("\n") || `${current.status}…`);
