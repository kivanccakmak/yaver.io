/**
 * domInspect.ts — the browser half of Yaver's DOM MODE (element inspect,
 ...
 * it — this dashboard — and we forward it over the session's own
 ...
 * React and no network, so all of it is testable (domInspect.test.ts).
 *
...
/** localStorage key for whether DOM mode was left on. Default OFF — DOM mode
 *  is opt-in by design ("off until explicitly enabled"), unlike screen context
 *  which is ambient. Per-browser, not per-project. */
 ...
 * overwhelmingly common case: the dashboard's own sandbox bridges, React
 * DevTools, browser extensions and the OAuth popup all post into this window.
 * A parser that threw, or that trusted `data.el`, would turn any of those into
...
// (mobile/src/lib/domInspect.ts); the parity test in that file pins it, and it
// is the half a hostile postMessage actually meets. Everything BELOW is where
// the two surfaces legitimately differ: the browser has a synchronous
// localStorage, React Native does not. Same exported names, same defaults, same
// meaning of "off" — different store.

/** Read whether DOM mode was left on. Default OFF — opt-in, never default. */
export function isDomInspectEnabled(): boolean {
  try {
    if (typeof window === "undefined" || !window.localStorage) return false;
    return window.localStorage.getItem(DOM_INSPECT_PREF_KEY) === "1";
  } catch {
    return false;
  }
}
...
export function setDomInspectEnabled(on: boolean): void {
  try {
    ...
    window.localStorage.setItem(DOM_INSPECT_PREF_KEY, on ? "1" : "0");
  } catch {
