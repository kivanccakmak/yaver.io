 ...
 *
 * TWIN of mobile/src/lib/approveFailureMessage.ts, byte-for-byte below the
 * header. Web cannot import from mobile/, so this is a copy — and a copy is a
 * drift risk by construction, which is why approveFailureParity.test.ts diffs
 * the two function bodies and fails when they disagree. Change one, change both.
 *
 * Tests: web/lib/approveFailureParity.test.ts
 */
